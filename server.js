...
const verifyApiKey = (req, res, next) => {
  ...
  if (!apiKey || apiKey !== API_KEY) {
    return res.status(401).json({ error: true, details: 'Unauthorized: Invalid API key' });
  }
  next();
};
app.use('/api/fetchCoinData', verifyApiKey);
...
app.get('/api/fetchCoinData/:id', async (req, res) => {
  let coinId = cleanCoinId(req.params.id);
  const auditLink = req.query.auditLink || '';
  ...
  try {
    ...
    const results = { data: {}, errors: [], risks: [], socials: {}, community: {}, developer: {}, tokenomics: {}, liquidity: {}, market_sentiment: {} };
    ...
    const apis = [
      {
        ...
        fetch: async () => {
          ...
          results.socials = {
            twitter: data.links?.twitter_screen_name ? { handle: `@${data.links.twitter_screen_name}`, url: `https://x.com/${data.links.twitter_screen_name}`, verified: true } : { verified: false },
            facebook: data.links?.facebook_username ? { handle: data.links.facebook_username, url: `https://facebook.com/${data.links.facebook_username}`, verified: true } : { verified: false },
            instagram: data.links?.instagram_username ? { handle: data.links.instagram_username, url: `https://instagram.com/${data.links.instagram_username}`, verified: true } : { verified: false },
            telegram: data.links?.telegram_channel_identifier ? { handle: data.links.telegram_channel_identifier, url: `https://t.me/${data.links.telegram_channel_identifier.replace('@', '')}`, verified: true } : { verified: false },
            discord: data.links?.chat_url?.find(url => url.includes('discord')) ? { url: data.links.chat_url.find(url => url.includes('discord')), verified: true } : { verified: false },
            reddit: data.links?.subreddit_url ? { handle: data.links.subreddit_url.split('/').slice(-2, -1)[0], url: data.links.subreddit_url, verified: true } : { verified: false },
          };
          results.community = {
    ...
    if (Object.keys(results.data).length === 0) {
      console.error(`No data fetched for "${coinId}" from any API`);
    ...
    if (!isNativeCoin && contractAddress && (chain === 'ethereum' || chain === 'bsc')) {
      try {
        const apiUrl = chain === 'ethereum'
          ? `https://api.etherscan.io/api?module=token&action=tokenholderlist&contractaddress=${contractAddress}&page=1&offset=10&apikey=${ETHERSCAN_API_KEY}`
          : `https://api.bscscan.com/api?module=token&action=tokenholderlist&contractaddress=${contractAddress}&page=1&offset=10&apikey=${BSCSCAN_API_KEY}`;
    ...
    res.status(500).json({ error: true, details: `Server error while processing "${coinId}". Please try again.` });
  }
});
